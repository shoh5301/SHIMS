...
int main(int argc,char** argv){
    ...
    if(n_rank==0){
	printf("=============================================================\n");
	printf("!  Stochastic HIgh-speed Microstructure Simulator           !\n");
	...
	while(1){
		...
		printf("  3 / d = Database Checker\n");
		printf("  4 / t = Auxiliary Tools: Graphics & Measurement\n");
		printf("  0 / q = Quit\n  >> ");
		scanf("%c",&func);

		     if(func=='3' || func=='d')
			DB_main(melt,jc,rscale);
...
//				    return 0;
	    ...
	    if((func=='2' || func=='s')){ // to avoid MPI issue
	      if(n_size==1){
		switch((int)rscale[0]){
		    case 0: // GG sim, w/o realistic scale
			...
			break;
		    default:
			printf("Simulation function error - rscale[0] %d\n",(int)rscale[0]);
			break;
		}
	      }else{
		switch((int)rscale[0]){
...
// COMMON INPUT OPTIONS
	...
	if(strcmp(option,"NONE")==0){ // input materials parameter manually
		...
		printf(" @@ NOTE: for pure materials, Liquidus == Solidus\nSolidus temperature (K) = ");
		scanf("%lf",&melt[0]);
		if(rscale[2]!=melt[0]){
		    printf("T0 (K) =");
		    scanf("%lf",&melt[8]);
	    ...
	    printf("Max Grain Boundary Energy for Misorientation?(J/m2)\n >> ");
	    scanf("%lf",&jc[1]);
	    printf("GB Energy Anisotropy Level(Degree) = "); // GBE information
	    scanf("%lf",&jc[3]);
	    ...
	    if(rscale[0]!=0){
		printf(" L^2 - L0^2 = K0_e * EXP( - Qe / R T ) * t\n");
		printf("                 K0_e (m^2/s) = ");
		scanf("%lf",&rscale[5]);
		printf("\n Lmc^2 - L0^2 = Kmc_0 * EXP( - Qmc_0 / R T ) * MCS\n");
		printf("                  Kmc_0 (site^2/s)= ");
		scanf("%lf",&rscale[7]);
		printf(" Solidification model, t = beta * MCS\n beta = ");
		scanf("%lf",&rscale[10]);
	    }
	}else{
...
// WETTING ANGLE
		...
		rscale[13]=1E3; //MAXIMUM TIME SCALING LIMIT <1000
	    }else{
	      rscale[12]=86400;
	      rscale[13]=1000;
	    }
...
# YES = Y, NO = N\n\
# USE \"#\" FOR ANNOTATIONS\n\
# NO MORE THAN 100 CHAR IN ONE LINEs\n\
##################################################################################################\n\
# SIMCARD\n\
#\n\
...
#        NOTE: automatically FDM = N when REAL = N\n\
...
#        (A = X,Y,Z, N = U,L // VALID ONLY WHEN B** = C, unit W/m2 K)\n\
...
# PTCL: Fraction of IMC particle? (UNIT: Number of particle voxels / Liquid voxels // DEFAULT = 0)\n\
...
# OPT:  (ONLY VALID WHEN TIME == 3) # of MCSs to modifiy length of MCS (DEFAULT = 1 MCS)\n\
...
#        ex) 11480*(1-T/1728)\n\
#	 Enter 0 for activate the DEFAULT option\n\
...
# POS:  First position for LASER irradiation (X Y, unit voxel from origin)\n\
#       ex) POS = 50 50\n\
...
# GEO:  Geometry factor for teardrop shape (0 < GEO < 5)\n\
