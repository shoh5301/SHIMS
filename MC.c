...
void MCtrial_SOL(int i,int j,int k,int**** grid,double**** fdmt,int dir[],int pbc[],int mode[],double melt[],double Tmax[],double dHfus[],double jc[],double tbc[][2],double Vsite,double Asite,double factor[],double Nsite[],double pcps[],double Qgg,char dfeq[]){
...
//		    dGfus=solidi_df(dHfus[0],Tnow,Tmax[0],Tmax[2],melt[8]); //>0 for prefered reaction
		    dGfus=Xl_df(dfeq,Tnow,dHfus[0],Tmax,melt[8]);
...
//		    prob=factor[1]*P_nucl(1/Tnow,dGfus,jc[4],fhetero_s,Nsite[0],dGfor,pcps,dGfus*Vsite);
		    ...
		    }else if(situ>0){// heterog. nucl. @ S-L interface / powder surface
...
//			prob=factor[1]*P_nucl(1/Tnow,dGfus,jc[7],fhetero_s,Nsite[1],dGfor,pcps,dGfus*Vsite);
		...
		}else if(grid[l][m][n][0]!=LIQORI && grid[l][m][n][0]!=MPISURF){ // && grid[l][m][n][0]!=PTCLORI 
...
/////////////////////////////////// solid-liquid interaction: solid epitaxial growth ////////////////////////////////////////
	...
	}else if(grid[i][j][k][1]!=POWORI){// && :grid[i][j][k][0]!=PTCLORI 
// target voxel is solid
//	    if(Tnow<=Tmax[0]){
		if(grid[l][m][n][0]!=LIQORI && grid[l][m][n][0]!=MPISURF && grid[l][m][n][1]!=POWORI){ // && grid[l][m][n][0]!=PTCLORI
		    // T < Tm: GRAIN GROWTH
...
//	f_IFE=dendrite_dir(now,(int)jc[7],jc[2]); // return aniso. factor for IFE
	...
	for(l=25;l>=0;l--){
		...
		if(tbc[n][0]<0 || tbc[n][1]<0) // tbc[][0]<0 -> convection // tbc[][1]<0 -> vapor condition
			...
			ten=ten+jc[4]*cos(DTOR*tbc[m][1]); // IFE_s-l * cos theta
	    }else{ // ASSUMPTION: powder also have the same IFE with l (g_s-l == g_p-l)
		nei26sel(l,&i,&j,&k,dir,pbc);
...
void LatHeatDiffus(double* T,double *L,double dT){
	if(*L>0){ // Latent heat release
	    if((*L)*(*L-dT)<0){ // reminaing *L < dT -> adjust dT
		*T+=*L;
		*L=0;
	    }else{
		*T+=dT;
	...
	}else if(*L<0){ // Latent heat absorption
	    if((*L)*(*L-dT)<0){ // remaining *L < dT -> adjust dT
		*T-=*L;
		*L=0;
	    }else{
		*T-=dT;
...
double LatHeatPerStep(double dT,double dtfdm,double dx,double k,double Vm,double Cp){
	double temp;
...
//	temp=dT*dtfdm*k*Vm/(dx*dx*Cp);
//	if(dx>2*DenTipRad)
	    temp=dT*dtfdm*k*Vm/(dx*dx*Cp)*dx/(2*DenTipRad);
//	else
//	    temp=dT*dtfdm*k*Vm/(dx*dx*Cp);

	return (temp<dT)? temp:dT;
}
...
double powcon(double tpow,int ori){
	return (ori==POWORI)? tpow:1.0;
}
...
/*		if(Tinfo[0]==TNOLIQUID){ // SOLID X, LIQUID X -> impossible
	...
	}else if(rtim<=rscale[12] && rtim<=rscale[13]*dtsave[1]){ // Solving FDM, acceleration available
	    if(dtsave[1]!=rtim){
		melt[1]=dtsave[0];
		if(rtim<melt[1]){ // fdm time > MC time
		    melt[1]=rtim;
